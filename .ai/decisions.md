...
- 2026-08-20: Admin Traffic is cached in KV (`analytics:traffic:v1`). Cloudflare GraphQL is not called on every dashboard load. Fresh 15 min, background refresh until 24 h, cron `*/10`. No public Cache Rule on `/api/admin/activity`.
- 2026-08-20: Inbound/outbound mail uses hosting SMTP `mail.dynasai.ai` (`192.250.229.217`, user `hello@dynasai.ai`). Do not point Cloudflare apex A at that IP (site stays on the Worker). Email Routing is disabled; Cloudflare MX is `mail.dynasai.ai` priority 0. Hosting DKIM selector is `default._domainkey`. `webmail.dynasai.ai` is proxied for Cloudflare SSL; `mail.dynasai.ai` stays DNS-only for SMTP/IMAP. Contact form emails the owner (`LEAD_NOTIFY`) and CC list (`LEAD_CC`, comma-separated) with form details, and sends the submitter a 2–3 working-day thank-you via SMTP. Re-enable Routing later on a paid plan if needed.
