...
- 2026-08-20: Inbound/outbound mail uses hosting SMTP `mail.dynasai.ai` (`192.250.229.217`, user `hello@dynasai.ai`). Do not point Cloudflare apex A at that IP (site stays on the Worker). Email Routing is disabled; Cloudflare MX is `mail.dynasai.ai` priority 0. Hosting DKIM selector is `default._domainkey`. Re-enable Routing later on a paid plan if needed.
